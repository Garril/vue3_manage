...
const store = createStore<IRootState>({
  ...
  getters: {
    getProperty(state) { // 没必要这样写，但是这只是为了示范
      return (keyName: string) => {
        return (state as any)[`${keyName}`]
      }
    },    
  },
...
obj.install = function(Vue) { // 默认会把 Vue 作为参数传过来
  ...
  Vue.prototype.$toast = toast; // 这里赋值注意，应该为10行创建的组件,
}
...
首先，搭一个最底部的架子，之后要去写其他的组件，只需要通过 `props`传入`options`就行

`base-echart 和 useEchart`帮我们把 `echart`实例，`resize`，`resize的监听`，`update后的resize`等均完成了
...
onMounted(() => {
  // ref 传入 useEchart拿到 setOptions函数
...
基于`base-echart`，传入要渲染的`echart`的基础的`options`，以及请求到的数据，实现不同的`echart`图表

（`line-echart、map-echart、pie-echart、rose-echart`均差不多，略）
...
// 小部分数据，再往上，props传入
// 但是title什么的，可以单独封装一个组件，显示title和下面的slot
const props = withDefaults(
  defineProps<{
    title?: string
    xLabels: string[]
...
效果：写配置 `xxx.config.ts` + `store`相关数据的保存，请求改变（`store中发生了数据请求`）。然后就能生成对应页面

思路：

首先，我们要清楚，最底部的组件，其实还是用的 `el-table、el-pagination`等组件

这个封装的 “最底部的组件 ”`table`如下：

### table -- the inner cpn

简单一看，我们可以知道，在`table`中，我们是，接受 很多的 `props`，

然后事件的处理，全`emit`出去，让父组件去处理，而且放开了几个 `slot`
...
<style scoped lang="less"> 略 </style>
```

### page-content

page-content 主要根据传入的 props，通过 store.getters 或者 store.dispatch 去发请求

从而 在store里获取数据，拿到 page-content中，才传入 table组件

按钮的权限控制，就在page-content中 v-if实现 

同时如果要 不通过 store dispatch发送请求，而改变数据，直接 emit 让父组件处理



排除掉`table组件`头部的`header`还有 底部的 `el-pagination`，我们就看`el-table的slot`，其核心如下：
...
可以看到，最核心的就是底部的 `template v-for`，

`propList`是从`table的props: contentTableConfig`解构出来的，依据它遍历生成了很多的`slot`
...
然后让我们看看，在 `page-content`是怎么用的`slot （g-table 就是 上面我们写的 table 组件）`
...
在`status createAt updateAt handler` 的下面，对其余的 传入的 插槽名 （`otherPropSlots`）进行了遍历插入

接着对于`g-table`也就是 table，我们在 page-content 传入的props 有四个：

`listData、listCount、contentTableConfig 和 pageInfo`，他们代表的含义如下

#### （ listData、listCount、contentTableConfig 和 pageInfo ）

其余的`page-content`内容如下：

```ts
// 1.双向绑定pageInfo -- 底部，换页要加载
const pageInfo = ref({ currentPage: 1, pageSize: 10 })
watch(pageInfo, () => getPageData())
// 3.从vuex中获取数据
const dataList = computed(() => store.getters[`system/pageListData`](props.pageName) ) // listData
const dataCount = computed(() => store.getters[`system/pageListCount`](props.pageName) ) // listCount
// contentTableConfig 从config文件夹内 import
// 4.获取其他的动态插槽名称
...
比如说，当前组件为`views/main/product/goods.vue`，要显示商品页面

在使用 `page-content`组件的时候，有两个必传的属性`contentTableConfig 和 pageName` ，
...
`pageName` 之后会进行拼接，去发送请求，并且按照`pageName`拼接出的字段名字，保存到`store`

（发送请求是发生在`store`里面的，在`page-content`中，只是`dispatch`）
...
const getPageData = (queryInfo: any = {}) => {
  ...
  store.dispatch('system/getPageListAction', {
    pageName: props.pageName, // 传入拿到的pageName到store去dispatch
    queryInfo: {
